...
import { IServerReqObject, ISpUserPeriodData } from '../../controller/serverTypes';
...
* @return {Date}
*/
function weekToDate(year: number, week: number, firstDayofWeek = 1): Date {
...
const valueToSeconds = (value: number): number => {
  ...
  const _stringValue = value.toString();
  ...
  if (_stringValue.length === 3) {
    const [_hours, _minutes] = _stringValue.split(".");

    const _hoursInSec = Number(_hours) * 3600;
    const _minInSec = Number(_minutes) * 10 * 60;
...
/* compare for sort */
const compareWeekPeriod = (_weekA: ISpUserPeriodData, _weekB: ISpUserPeriodData): number => {
